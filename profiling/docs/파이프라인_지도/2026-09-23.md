...
| | |
|---|---|
| 기준 코드 | 브랜치 `integration/v1-profiling-search` · 커밋 `4970014`(평탄화 → DB 전용화 → 카탈로그 DB 적재 → 시퀀스 문서 → 재고 3값) |
| 범위 | **v1** — 비선호 카테고리만 반영. 취향 문장·리뷰를 읽는 모델·검증기는 v3 |
| 동작 | 7.6 접수 → 202 → 백그라운드 → 재고 없음(`unavailable`)·비선호 제외 → 조회수순 30개 → 7.7 콜백 → 결과를 DB에 기록. 09-22에서 바뀐 동작은 **재고 필터 한 줄뿐**이다 |
| 저장소 | PostgreSQL **하나뿐** — `ai_profile.profile_runs` · `ai_profile.recipient_profiles`. 메모리 구현과 `PROFILING_STORE` 설정은 제거했다. 카탈로그도 `ai_catalog`(0003)에서 읽는다 — `PROFILING_CATALOG_SOURCE=db`. 로컬 기본값은 아직 파일 |
| 테스트 | `uv run pytest -q` → **94 passed, 2 skipped**(v3 stub). 단위 65(DB 없이) · 통합 31(진짜 PostgreSQL, 없으면 skip) |
...
| 구현(어댑터) | `catalog.py` · `stores.py` · `backend.py` | `DbCatalogReader`(배포)·`FileCatalogReader`(로컬) / `DbProfileRunStore`·`DbRecipientProfileStore` / `HttpBackendPort` |
...
| 바깥 | | `tests/fixtures/catalog_sample.json`(111건) · PostgreSQL `ai_chat`(alembic 0001·0002 `ai_profile` · 0003 `ai_catalog` 4,231건) · Backend — 로컬은 `tools/fake_backend`(:8081) |
...
| **구현 전체 시퀀스 문서** — 기동 · 성공 전체 · 접수 거절 · 분석 실패(침묵) · 콜백 4갈래 · 슬롯, mermaid 6장 | 계약(BE 연동 필드표)과 별개로 *우리 쪽 내부 순서*를 남길 곳이 없었다 | `docs/시퀀스_전체.md` · `docs/assets/seq/` · `docs/assets/build_be_sequences.py`(대상 문서 목록을 `targets()`로) |
| **카탈로그를 DB에서 읽는 구현** — `DbCatalogReader`. 활성 버전 id만 묻고 바뀌었을 때만 다시 읽는다. Backend 번호가 없으면 수집처 ID 숫자부를 임시 번호로 쓰고 `/health`에 `provisional_ids` 표시 | 배포 선결 조건 — 컨테이너 안에는 카탈로그 파일이 없다. 그리고 `catalog_version_id`가 파일 고정 UUID가 아니라 진짜 DB 버전이 된다 | `catalog.py` · `settings.CATALOG_SOURCE` · `main.lifespan` · `main.health` · 통합 시험 6개 |
| **재고를 3값으로** — `available`(있음) · `unavailable`(없음) · `unknown`(정보 없음). 추천 풀은 `unavailable`만 제외 | 09-23 Backend 합의. 검색기와 같은 형이고, 실제 재고를 아는 Backend가 최종 판단한다. 패키지 적재분은 전건 `unknown`이라 이 규칙이 아니면 풀이 0건 | `schemas.py`(`Availability`·변환 검증기) · `pipeline.build_pool()` · `catalog.py` · `0003_catalog.py`(`availability TEXT` + CHECK) · `load_catalog.py` · `fetch_export.py` |
...
- `uv run pytest -q` → **94 passed, 2 skipped**
- 단위가 정말 DB 없이 도는지: `PROFILING_DATABASE_URL="postgresql+psycopg://x:x@127.0.0.1:1/none" uv run pytest tests/unit -q` → 63 passed, 2 skipped
- 통합이 DB 없으면 건너뛰는지: 같은 환경변수로 `uv run pytest tests/integration -q` → 31 skipped
...
|---|---|---|---|---|---|
| 1 | **Backend ID 회신 반영** — Backend가 DB 적재를 끝내고 매핑을 알려주면 `product-id-map.jsonl`·`category-id-map.jsonl`을 받아 채운다 | 7.7로 보내는 30개가 Backend에 없는 번호면 화면에 아무것도 안 뜬다. 지금 `backend_product_id`는 0/4,231 | `uv run python -m tools.catalog.load_catalog --id-map … --category-id-map …` | 적재 CLI 보고에서 `Backend ID 상품 4231/4231`, 경고 문구가 사라짐 | 한 시간 (Backend 회신 필요) |
| 1′ | **남은 Backend 미결 정리** — 7.7 태그, 7.9 조회수 필드명·정렬 기준, 제외 vs 감점, 비선호 ≤5 잠금 | 재고는 09-23에 정해졌고(3값) 이 넷이 남았다 | `schemas.py`(`ProfileCallbackRequest`·`ProductRecord.viewCount`·`dislikedCategories max_length`) · `pipeline.build_pool()` · `tools/fake_backend` | 콘솔 7.6 → 7.7 정상, `uv run pytest -q` 전부 통과, `이름_대조표.md` 갱신 | 각 한 줄 |
| 2 | **7.9 export로 재고·조회수 채우기** | 패키지에는 재고 정보도 조회수도 없다 — 지금 `availability`는 4,231건 전부 `unknown`, `view_count`는 NULL. Backend export가 와야 풀 정렬이 의미를 갖는다 | `tools/catalog/fetch_export.py` → `ai_catalog.products` 갱신 경로 추가 · Backend에 "재고를 모르는 상품은 `available`을 빼거나 `null`로" 전달 | 적재 CLI 보고에서 `availability` 확인 건수 > 0, `view_count` 있음 > 0 | 반나절 (Backend export 필요) |
...
| 6 | 팀원 레이아웃으로 합치기 (uv 단일 pyproject, 루트 `app/`) | 09-21 보류 결정. 배포 전 필요 | `src/profiling/*` → `app/profiling/*` · `pyproject.toml` 통합 · import 경로 · CI | `uv run pytest -q`가 합친 트리에서 그대로 통과 | 반나절 |
...
| 8 | ~~카탈로그를 파일 → DB로 교체~~ **완료 (09-23)** — 남은 것은 로컬 기본값을 `db`로 바꿀지 결정 | — | `catalog.DbCatalogReader` · `CATALOG_SOURCE` | `PROFILING_CATALOG_SOURCE=db` 로 앱이 뜨고 `/health`가 활성 버전 UUID·4,231건을 보고한다 (통합 시험 6개로 고정) | 완료 |
| 9 | v3 — 마이그레이션 0004 · 검증기 이식(실험 `2_validate.py`) · `ProfileModel` 구현(규칙 추출기 1안, Ollama 2안) · Search 호출 | 취향·리뷰 반영 | `alembic/versions/0004_*.py`(0003은 카탈로그가 씀) · `pipeline.profile()` 2단계 · `model.py` 신설 · `ports.py`에 `ProfileModel`·`Embedder` 다시 추가 · `types.merge_explicit_dislikes`·`to_search_hint` | 실험 하네스와 같은 결과(Recall@30 71) · skip 3개 해제 | 여러 날 |
...
막히면 볼 것: [README.md](../../README.md)(구조·실행·테스트) · [코드_안내서.md](../코드_안내서.md)(파일·함수별 역할) · [DB_ERD.md](../DB_ERD.md)(표 구조·키·관계) · [DB_전환_설명.md](../DB_전환_설명.md)(테이블·저장 시점) · [이름_대조표.md](../../이름_대조표.md)(같은 뜻 다른 이름) · [BE_연동_필드표.md](../BE_연동_필드표.md)(BE 전달본) · [시퀀스_전체.md](../시퀀스_전체.md)(구현 전체 순서 6장) · 설계 근거는 `KTB4_12team/AI/latest/01_모델_API_설계/담당파트_상세설계서_7.6_7.9.md`와 위키 `서비스-아키텍처-구현-상세` §10·§16.
