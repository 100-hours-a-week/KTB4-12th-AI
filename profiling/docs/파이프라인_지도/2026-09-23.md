# 파이프라인 지도 — 2026-09-23 (구조 평탄화: 6폴더 → 1폴더)

| | |
|---|---|
| 기준 코드 | 브랜치 `integration/v1-profiling-search` · 커밋 `d2c3f41` |
| 범위 | **v1** — 비선호 카테고리만 반영. 취향 문장·리뷰를 읽는 모델·검증기는 v3 |
| 동작 | 7.6 접수 → 202 → 백그라운드 → 비선호 제외·조회수순 30개 → 7.7 콜백 → 결과를 DB에 기록. **09-22와 같음 — 동작은 하나도 안 바뀌었다** |
| 저장소 | PostgreSQL(`docker compose`) — `ai_profile.profile_runs` · `ai_profile.recipient_profiles`. 카탈로그는 아직 파일 |
| 테스트 | `uv run pytest -q` → **75 passed, 3 skipped**(v3 stub). 평탄화 전과 같은 수 |
...
| ③ 처리 | `pipeline.py` | `profile()` — **0** `store.save(RUNNING)` → **1** `catalog.active()` → **2** `needs_model()`(v1은 False) → **3** `build_pool()`(판매중·비선호 제외·조회수↓·30) → **4** `ProfileOutcome(RESULT_READY)` → **5** `store.save()` → **6** `recipient_store.upsert(from_outcome())`. 실패는 `_fail()` → FAILED, 콜백 없음 |
...
| 구현(어댑터) | `catalog.py` · `stores.py` · `backend.py` | `FileCatalogReader` / `DbProfileRunStore`·`DbRecipientProfileStore`·`MemoryProfileRunStore`·`MemoryRecipientProfileStore`(목) / `HttpBackendPort` |
...
| 저장소 4파일을 `stores.py` 하나로 | 같은 계약의 DB 판·메모리 판이 나란히 보이게. 이름 충돌은 `RUNS_TABLE`/`PROFILES_TABLE`로 해소 | `stores.py` |
...
- `uv run pytest -q` → **75 passed, 3 skipped** (평탄화 전과 같은 수 — 로직이 안 바뀌었다는 증거)
- `uv run python -c "import profiling; print(profiling.__all__)"` → 공개 표면 7개
- `uv run ruff check src tests tools alembic` → `stores.py`의 C408 1건만(아래 §4)
...
| 상품·카테고리 ID 타입 | **미결** | Backend는 `BIGINT`. 팀원 검색기는 `product_id TEXT`·`category_id TEXT`, 수집본은 `KAKAO_GIFT:10002797`·`CAT-01-02`. **양쪽 다 Backend id가 아니다** — Backend 상품 목록으로 재적재해야 함 |
| `MemoryRecipientProfileStore` | 골격만 | `stores.py` 맨 아래. `ruff` C408 1건은 이 목 코드 — 채울 때 같이 정리 |
...
| 6 | `MemoryRecipientProfileStore` 채우기 | STORE=memory에서 프로필 저장을 건너뛰고 있음 | `stores.py` 맨 아래 — `upsert`(should_replace 규칙)·`get`·`delete` · `main.py` memory 분기 한 줄 | `tests/unit/test_recipient_profile.py::test_memory_store_upsert_get_delete` skip 해제 후 통과 · `ruff` 0건 | 1시간 |
